---
title: "How to build a generator-to-file pipeline"
sidebar_position: 0
---
...
a [pipeline configuration file](/docs/pipeline-configuration-files/getting-started)!
...
```bash
mkdir pipelines && touch pipelines/generator-to-file.yaml
```
...
├── conduit
└── pipelines
    └── generator-to-file.yaml
```


## Step 3: The pipeline's basics

Next, write the following to `generator-to-file.yaml`:
...
pipelines:
    - id: pipeline1
      ...
      status: running
      name: pipeline1
      description: A generator-to-file pipeline
      # The connectors will be configured in the following steps
      connectors:
...
Now we can add a source connector under `connectors` in the configuration file:
...
The configuration below instructs the generator to produce 1 record with a
structured payload. The payload will contain two fields: `id` (an integer) and
`name` (a string).
...
The file `generator-to-file.yaml` has the following content:
...
about [pipeline configuration files](/docs/pipeline-configuration-files/getting-started)
or other [features](/docs/features) of Conduit.