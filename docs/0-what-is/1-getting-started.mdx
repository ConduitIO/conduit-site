...
In this guide, we'll learn how to install Conduit and use it Conduit to build
and run a pipeline. The pipeline will be continuously generating sample user
data (using the
...
The easiest way to get started with Conduit on your macOS or Linux system is to
install it using our installation script. Simply run the following command
in your terminal:
...
```shell
$ conduit --pipelines.path pipeline-generator-to-log.yaml
```
...
Looking for more examples? Check out the examples in
