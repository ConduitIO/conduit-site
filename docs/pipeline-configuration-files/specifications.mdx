...
- **Data Type**: String
- **Required**: No
- **Default**: `True` (all records will passed to the processor)
- **Description**: a Go template expression that will be used as a condition to pass the record to the processor or not.
the Go template expression will be evaluated, and the evaluated value should either be `True` or `False`, `True` means
the record will be passed to the processor, `False` means it will be skipped.
