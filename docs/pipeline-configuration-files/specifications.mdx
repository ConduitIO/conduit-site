...
:::info
This document describes configuration files with version 2.0. Configurations
...
processors please update it to version 2.0 to ensure that processors are ordered
correctly.
:::
...
**Warning**: Changing this property will cause all connectors to start from the
beginning.
...
- **Description**: This node references the connector plugin used for storing
  dead-letters. See how to [reference a connector](/docs/connectors/referencing).
...
- **Description**: Defines if the connector is a source or a destination. A
  working pipeline needs at least one source and one destination.
...
- **Description**: A map of configuration keys and values for the connector
  plugin. The values in this map depend on the chosen connector [`type`](#type)
  and [`plugin`](#plugin-1).
