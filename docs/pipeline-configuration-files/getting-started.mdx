...
It's as simple as creating a YAML file that defines pipelines, connectors, processors, and their corresponding
configurations.

## Example pipeline (file to file)

Create a folder called `pipelines` at the same level as your Conduit binary. Inside of that folder create a file named
`file-to-file.yml`.
...
Now start Conduit and notice the log line saying that the pipeline `file-to-file` was created:
...
Conduit is now running the pipeline `file-to-file` which continuously reads lines added to file `./example.in` and
copies them to file `./example.out`. Try writing a line to `./example.in` and checking the content of `./example.out`.
