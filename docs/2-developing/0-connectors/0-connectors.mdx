...
Go. Using the Go SDK is the recommended way of writing a Conduit connector.

There is also a [Python SDK](https://github.com/ConduitIO/conduit-connector-sdk-python)
(**pre-alpha**) — a Python connector speaks the same gRPC protocol and runs as a
first-class standalone plugin, which is handy when the system you're integrating
has a mature Python client. See
[Write a Connector in Python](/docs/developing/connectors/writing-a-connector-in-python).

If you are implementing a connector in another language you should refer to
...
The fastest way to get a building copy of that template locally is
[`conduit connector new`](/docs/developing/connectors/scaffold-new), which
scaffolds it for you in one command (module rename, code generation, a verified
`go build`, and git init).
