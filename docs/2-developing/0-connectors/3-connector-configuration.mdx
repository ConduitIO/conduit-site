...
As it can be seen from the example, every configuration struct needs to embed
either `sdk.UnimplementedSourceConfig` or `sdk.UnimplementedDestinationConfig`.
...
```go
type Config struct {
    TableConfigs map[string]CollectionConfig `json:"tables"`
}
