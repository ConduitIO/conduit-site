...
`s.config` needs to be a struct that embeds `sdk.UnimplementedSourceConfig`.

We recommend that this struct embeds `sdk.DefaultSourceMiddleware`, which is a
configuration struct through which the following functionality is automatically
added to the source:
