...
([`conduit-connector-sdk-python`](https://github.com/ConduitIO/conduit-connector-sdk-python),
**pre-alpha**); until scaffolding wires it in, start a Python connector by hand — see
[Write a Connector in Python](/docs/developing/connectors/writing-a-connector-in-python).
:::
...
| Flag | Description |
| --- | --- |
| `--lang` | Target language. Only `go` produces a scaffold; any other value (including `python`) fails with an explicit "not supported yet" error. A Python SDK exists ([pre-alpha](/docs/developing/connectors/writing-a-connector-in-python)); the scaffolder just doesn't generate for it yet. Default: `go`. |
