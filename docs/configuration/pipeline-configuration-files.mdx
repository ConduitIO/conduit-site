...
To create processors, either add a `processors` map under a pipeline, or under a connector, depending on its parent.
Check this YAML file example with explanation for each field:

``` yaml
version: 1.1                    # Parser version [optional]
  ...
  pipeline1:                    # Pipeline ID, has to be unique.
    ...
    connectors:                 # A map of connectors IDs and their configurations.
      con1:                     # Connector ID, has to be unique per pipeline.
        ...
        settings:               # A map of configuration keys and values.
      ...
      con2:
        ...
        processors:             # A map of processor IDs and their configurations, "con2" is the processor parent.
          proc1:                # Processor ID, has to be unique for each parent
            ...
            settings:           # A map of processor configurations and values
              Prop1: string
    processors:                 # Processor IDs, that have the pipeline "pipeline1" as a parent.
      proc2:
        type: js
        settings:
          prop1: ${ENV_VAR}     # You can use environment variables by wrapping them in a dollar sign and curly braces ${}.
    dead-letter-queue:          # Dead-letter queue configuration. [optional]
      plugin: "builtin:file"    # Connector plugin. [optional, default=builtin:log]
      settings:                 # A map of configuration keys and values for the connector.
        path: "./dlq.out"
      window-size: 5            # Nack window size. [optional, default=1]
      window-nack-threshold: 2  # Nack window threshold. [optional, default=0]
```
...
If two pipelines in one file have the same ID then the file would be
