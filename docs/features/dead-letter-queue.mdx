...
Below is a visual representation a sliding window monitoring 5 records with the
nack threshold set to 2. Notice that once the threshold is exceeded the pipeline
stops.

![nack window animation](/images/dlq-window-animation.gif)

## Configuring a dead-letter queue

By default a Conduit pipeline doesn't have a dead-letter queue. You can enable
the dead-letter queue by changing the window size and nack threshold:

- If the window size is >1 and nack threshold is 0, then the first nacked record
  will cause the pipeline to stop running immediately. Such a configuration
  essentially disables the dead-letter queue. This is the default configuration.
- If the window size is set to 0, then no records are monitored and all nacked
  ...
  queue.
- If the window size is >1 and nack threshold is >0, then nacked records are
  sent to the dead-letter queue until the threshold is exceeded.
...
Please check
the [pipeline config file documentation](/docs/configuration/pipeline-configuration-files)
