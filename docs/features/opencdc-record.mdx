...
- `opencdc.collection` contains the name of the collection from which the record originated and/or where it should be written to.
...
The following metadata is only available in records once they are read by a **source plugin**:
...
The following metadata is only available in records once they are written by a **destination plugin**:
