...
Pipeline Recovery is enabled by default since Conduit v0.12. To have the
pre-v0.12 behavior, where a pipeline stops and goes into the `degraded` state
the first time it experiences an error, you should set the configuration
parameter `pipelines.error-recovery.max-retries` to 0.
...
time, `max-retries-window`. If `max-retries` has been set, then Conduit will
