...
time (the so-called `MaxRetriesWindow` parameter). If a limit on the number of
retries has been set, then Conduit will make sure that limit is not exceeded
over any `MaxRetriesWindow` period of time. In other words, the number of restart
attempts is reset to 0 after `MaxRetriesWindow`.
...
`max-retries-window` to 10 minutes (the default value is 5 minutes).
...
Let's consider a few scenarios and see how is the above configuration applied.
