...
For example, if we wanted to use a PostgreSQL instance listening on `localhost:5432` using the terminal directly:
...
This can also be done using configuration options in your [pipeline configuration file](/docs/pipeline-configuration-files/getting-started):
