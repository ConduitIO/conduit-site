...
When a record with structured data is processed, both schema subject and version are attached to the record metadata so it preserves all types when it comes to structured records. You can see examples on [`opencdc.payload.schema.*`](/docs/features/opencdc-record#opencdcpayloadschema).
