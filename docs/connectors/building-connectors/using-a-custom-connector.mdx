...
The Conduit Connector template includes a `Makefile` that can be used to generate a binary version of your connector
...
To run the Connector, you will need to put it in a directory that Conduit is able to access.  By default, this directory is the `connectors` directory located alongside the Conduit binary.  Move your newly created Connector binary into the `connectors` directory and run Conduit using the following command:
