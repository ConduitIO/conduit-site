...
Gathers data from the configured data source and formats it into a `opencdc.Record`
that is returned from the function. The returned `opencdc.Record` is queued into the
pipeline to be consumed by a Destination connector.
...
position was successfully processed. It's worth that while some source connector
need to implement this functionality (e.g. in the case of messaging brokers),
others don't have to (e.g. a file source).
