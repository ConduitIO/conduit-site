...
Most formats come in the form of `format-name/encoding`. `format-name` is the name of the converter that **controls the
structure** of the produced record. `encoding` is the encoding used for the structure created by the converter.
Currently, only `json` is supported. More encoders are planned in future (e.g. Avro, Parquet, ...)
