...
`sdk.record.format` is used to choose the format itself whereas `sdk.record.format.options`
...
Currently, `json` is supported. More encoders are planned in future (e.g. Avro, Parquet, ...)
