...
the [OpenCDC format](https://github.com/ConduitIO/conduit/blob/main/docs/design-documents/20220309-opencdc.md). Conduit
...
same or different pipelines can use different formats, which provides additional flexibility.

## Configuration parameters

There are two connector configuration parameters which control the output format: `sdk.record.format` and
`sdk.record.format.options`. `sdk.record.format` is used to choose the format itself whereas `sdk.record.format.options`
allows the user to control specifics of the chosen format.

## Available formats

Most formats (but not all) come in the form of `format-name/encoding`.

`format-name` is the name of the converter that **controls the structure** of the produced record. `encoding` is the
encoding used for the structure created by the converter. Currently, only `json` is supported. More encoders are planned
in future (e.g. Avro, Parquet, ...)
