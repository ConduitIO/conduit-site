...
:::caution
`sdk.schema.extract.payload.enabled` and `sdk.schema.extract.key.enabled` should be set to false when producing raw data (not structured) like the example below. 

If you're developing a connector, you can disable this automatically by updating the Connector's middleware. See more info about `NewSource()` when developing a source connector [here](/docs/connectors/building-connectors/developing-source-connectors/#newsource).
:::
