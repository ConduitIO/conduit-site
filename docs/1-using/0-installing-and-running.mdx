...
Every download is verified against the SHA-256 in the release's `checksums.txt` before anything is
installed. A checksum that does not match, or a `checksums.txt` the script cannot fetch, aborts the
install — it never installs an unverified artifact.
:::
