...
gRPC protocol. This means that the maximum size of a record is limited to **4 MB by
default**. If you try to send a record larger than this limit, you will see an error
...
:::tip
If you need to process larger records, you can increase the maximum
size of a record by setting the `-max-record-size` flag when starting Conduit.
:::
