...
The [OpenCDC format](/docs/core-concepts/opencdc-record) recommends that a record's
source or destination collection is written to its metadata using
the [`opencdc.collection` key](/docs/core-concepts/opencdc-record#opencdccollection).
