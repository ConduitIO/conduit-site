...
- A PostgreSQL source connector that monitors your support tickets table using Change Data Capture (CDC)
- OpenAI's GPT models to generate concise summaries
- An HTTP destination connector to send formatted notifications to Slack
...
The steps below will guide you through setting up your database, configuring the AI processing, and running the complete pipeline. If you're in a hurry, you can skip ahead and [run the pipeline](#step-7-run-the-pipeline) immediately.
...
1. **Generate summaries**: OpenAI analyzes each ticket message and creates a concise summary
2. **Rename fields**: Prepares the data format for Slack
3. **Format messages**: Creates an attractive Slack notification with ticket context
4. **Clean data**: Removes fields that aren't needed in the final notification
