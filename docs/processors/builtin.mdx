...
Replaces a field in the key in raw data with a schema or in structured data. Raw data without a schema is not supported.
...
Works in the same way as [timestampconvertorkey](#timestampconvertorkey), except that it's applied to `Record.Payload.After`.
