...
- `exists` (Optional)
  ...
  Gives the user a chance to define an existence query for a given filter. `exists` has an effect only if `conduit`
  doesn't pass.
  Possible values are: `fail`, `include`, `exclude`. Defaults to `fail`.

If `condition` passes, then it will handle the record as `type` dictates (i.e. keep the record, drop it, or return an
error).

If `condition` **doesn't** pass (i.e. no node in the payload matches the expression), then the record is handled as
follows:
- if `exists` is not configured, the record is dropped.
- if `exists` is configured and matches, the record is dropped.
- if `exists` is configured but doesn't match, then the record is handled as `missingornull` specifies.
