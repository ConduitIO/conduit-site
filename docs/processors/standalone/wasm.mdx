...
A standalone processor is a plugin that is compiled into [WebAssembly](https://webassembly.org)
(Wasm) and interacts with Conduit using two host functions: `command_request` and
