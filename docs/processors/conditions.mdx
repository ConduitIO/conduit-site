...
:::warning
This condition will use the [Go templates](https://pkg.go.dev/text/template) format, and when returned value of this expression returns **true**, then the processor will be executed, otherwise it will be skipped. This effectively means that by default all records will be processed unless the condition is met.
:::
