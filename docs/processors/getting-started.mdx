...
When it comes to using a processor either on a connector, or a pipeline, Conduit supports different types:
...
### Through a [pipeline configuration file](/docs/pipeline-configuration-files/getting-started)
...
### Through the [HTTP API](/api)

The processor endpoints live under the `/v1/processors` namespace, and if you'd like to use our [HTTP API](/api) to attach a processor to either connector or a pipeline, here's how you could do it.
