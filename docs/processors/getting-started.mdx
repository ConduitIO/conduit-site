...
familiar with [pipeline semantics](/docs/features/pipeline-semantics) is highly recommended.
...
Creating a processor through a pipeline configuration file can be done as below:
```yaml
- id: extract-name
...
In this example, we're using the built-in `extractfieldpayload` processor. It will replace a record's payload with the
value found in the `name` field.
...
Conduit provides a number of [built-in processors](/docs/processors/builtin), such as filtering fields, replacing them, posting payloads to HTTP endpoints etc.
