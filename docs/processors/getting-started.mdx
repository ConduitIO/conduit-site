...
The processor endpoints live under the `/v1/processors` namespace, and to attach a processor to either connector or a pipeline, you could simply do a `POST` request to `/v1/processors` specifying `parent.type` as `TYPE_PIPELINE` or `TYPE_CONNECTOR`. Default value is `TYPE_UNSPECIFIED`. 
