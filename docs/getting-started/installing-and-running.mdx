...
For an ARM based back Mac for example we'd grab the `conduit_0.11.0_Darwin_arm64.tar.gz` archive.

### unzip the archive

```shell
tar xzvf conduit_0.11.0_Darwin_arm64.tar.gz
```
...
```shell
$ TAG=v0.11.0; curl -o conduit.tar.gz -L https://github.com/ConduitIO/conduit/archive/refs/tags/$TAG.tar.gz
```
