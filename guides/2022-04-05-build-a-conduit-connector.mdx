...
In this article we are going to walkthrough step by step how to build a Conduit connector. 

Conduit connectors are ... Conduit Connectors can be a source, destination, or both. 
...
Connectors can be created in many programming languages because Conduit Plugins leverage GRPC. For this example, we are going to use the Golang SDK. 

Also, for this example, the datastore we are building the connector for is Algolia. Algola is TODO. As a destination, this connector allows for real-time syncing between any Conduit Source and Algolia.

To communicate with Algolia, we can use the [Algolia Golang library](https://www.algolia.com/doc/api-client/getting-started/install/go/?client=go).
...
- `cmd/algolia/main.go` - Entry point
...
`sdk.Serve` is TODO. Notice `nil` as the second parameter. This is due to this connector being just a Destination.
...
`Configure` is always the first function to be called in a connector and is used to store and validate the connector configuration. 
...
The `Object` methods serializes the record. This is implemented in `object.go`.
...
This will compile our connector to binary.
...
In this guide, we have covered the basics of how to build a connector for Conduit. The best way to learn more about what's possible with Conduit is to check out the [Conduit documentation](https://docs.conduit.io/), the [Conduit API documentation](https://docs.conduit.io/api/), and the [Conduit SDK](/).
